...
def call(String image_tag, String dockerfile_path, String context) {
  stage('Build Docker image') {
        try{
            // dockerImage = docker.build("${params.BASEIMAGE}", "${image_tag}", "-f ${dockerfile_path} ${context}")
            // dockerImage = docker.build("${image_tag} --build-arg BASEIMAGE=${params.BASEIMAGE} -f ${dockerfile_path} ${context}")
